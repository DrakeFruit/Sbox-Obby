...
public class PlayerController : Component, INetworkSerializable
{
	...
	[Property] public float JumpVolume { get; set; } = 2f;
	...
	protected override void OnUpdate()
	{
		...
		if ( !IsProxy )
		{
			...
			// Clamp the pitch angle to prevent the camera from going beyond 90 degrees up or down
			EyeAngles.pitch = Math.Clamp(EyeAngles.pitch, -90f, 90f);
	...
	public void OnJump( float floatValue, string dataString, object[] objects, Vector3 position )
	{
		...
		SoundHandle soundJump = Sound.Play(JumpSound, Transform.Position);
		soundJump.Volume = JumpVolume;
	}
